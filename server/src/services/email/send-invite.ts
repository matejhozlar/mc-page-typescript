...
/**
 * Generates the HTML email body for server invite
 *
 * @param discordName - Recipient's Discord username (displayed in greeting)
 * @param token - Unique verification token for Discord server access
 * @returns HTML string with embedded logo reference
 */
...
/**
 * Sends an invite email to a waitlist user and marks them as invited
 *
 * Generates a unique verification token, sends the invite email with
 * server details and Discord Link, then updates the database entry
 *
 * @param id - The waitlist entry ID
 * @returns Result object indicating success/failure with status code
 * @throws Does not throw - returns error result instead
 */
