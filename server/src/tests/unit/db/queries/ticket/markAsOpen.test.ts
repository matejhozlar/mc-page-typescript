...
describe("TicketQueries - markAsOpen", () => {
  let mockDb: Pool;
  ...
  describe("markAsOpen", () => {
    ...
    it("should log success message after marking as open", async () => {
      const mockResult = MockFactory.createMockQueryResult([], 1);
      (mockDb.query as any).mockResolvedValue(mockResult);
    ...
    it("should set status to 'open' in query", async () => {
      const mockResult = MockFactory.createMockQueryResult([], 1);
      (mockDb.query as any).mockResolvedValue(mockResult);
      ...
      expect(callArgs[0]).toMatch(/SET\s+status\s*=\s*'open'/i);
    });
    ...
    it("should throw error when ticket not found (rowCount = 0)", async () => {
      ...
      await expect(ticketQueries.markAsOpen("999999999")).rejects.toThrow(
    ...
    it("should throw error when rowCount is null", async () => {
      ...
      await expect(ticketQueries.markAsOpen("987654321")).rejects.toThrow(
    ...
    it("should pass correct channel_id parameter to query", async () => {
      ...
      await ticketQueries.markAsOpen("123456789");
    ...
    it("should handle empty string channel_id", async () => {
      ...
      await expect(ticketQueries.markAsOpen("")).rejects.toThrow(
    ...
    it("should handle very long channel_id", async () => {
      ...
      await ticketQueries.markAsOpen(longChannelId);
    ...
    it("should handle special characters in channel_id", async () => {
      ...
      await ticketQueries.markAsOpen("chan-!@#$%");
    ...
    it("should use parameterized query to prevent SQL injection", async () => {
      ...
      await ticketQueries.markAsOpen(maliciousInput);
    ...
    it("should update both status and updated_at fields", async () => {
      ...
      const callArgs = (mockDb.query as any).mock.calls[0];
      expect(callArgs[0]).toMatch(/status\s*=\s*'open'/i);
      expect(callArgs[0]).toMatch(/updated_at\s*=\s*NOW\(\)/i);
    });

    it("should handle concurrent reopens for different tickets", async () => {
      const mockResult = MockFactory.createMockQueryResult([], 1);
      (mockDb.query as any).mockResolvedValue(mockResult);

      await Promise.all([
        ticketQueries.markAsOpen("channel-1"),
        ticketQueries.markAsOpen("channel-2"),
      ]);
    ...
    it("should handle marking same ticket as open multiple times", async () => {
      const mockResult = MockFactory.createMockQueryResult([], 1);
      (mockDb.query as any).mockResolvedValue(mockResult);
    ...
    it("should include channel_id in error message", async () => {
      ...
      await expect(
        ticketQueries.markAsOpen("specific-channel-123")
    ...
    it("should handle unicode characters in channel_id", async () => {
      ...
      await ticketQueries.markAsOpen("chan-你好-🎮");
    ...
    it("should succeed when rowCount is greater than 1 (edge case)", async () => {
      ...
      await expect(
        ticketQueries.markAsOpen("987654321")
    ...
    it("should handle null-like string channel_id", async () => {
      ...
      await ticketQueries.markAsOpen("null");
    ...
    it("should work for reopening closed tickets", async () => {
      const mockResult = MockFactory.createMockQueryResult([], 1);
      (mockDb.query as any).mockResolvedValue(mockResult);
    ...
    it("should work for reopening deleted tickets", async () => {
      const mockResult = MockFactory.createMockQueryResult([], 1);
      (mockDb.query as any).mockResolvedValue(mockResult);
